...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const projectSliderContainer = document.getElementById('projects-slider');
    ...
    async function loadProjects() {
        ...
        try {
            const res = await fetch(`https://api.github.com/gists/${API_CONFIG.GIST_ID}`);
            const data = await res.json();
            const rawUrl = data.files['projects.json'].raw_url;
            const rawRes = await fetch(rawUrl);
            const projects = await rawRes.json();
    ...
    function renderProjectsSlider(projects) {
        ...
        const isFewCards = projects.length < 4;
        ...
        const renderList = isFewCards ? projects : [...projects, ...projects, ...projects];

        renderList.forEach(p => {
            const mediaUrl = p.image || '';
            const isVideo = mediaUrl.match(/\.(mp4|webm|mov|mkv)$/i);
            
            let mediaHTML = '';

            if (isVideo) {
                const videoSrc = mediaUrl.includes('#') ? mediaUrl : mediaUrl + '#t=0.001';
        ...
        const playButtons = projectSliderContainer.querySelectorAll('.custom-play-btn');
        ...
        function handleExitFullscreen() {
            if (!document.fullscreenElement && !document.webkitFullscreenElement) {
                const videos = projectSliderContainer.querySelectorAll('.project-video');
                videos.forEach(vid => {
                    vid.pause();
                    vid.controls = false; 
                });
    ...
    window.addEventListener('load', () => { if(preloader) preloader.classList.add('hide-preloader'); });
    ...
    const hideServices = document.getElementById('h-services');
    const hideContact = document.getElementById('h-contact');
    ...
    const stats = document.querySelectorAll('.stat-num-light');
...
const servicesData = [
    { title: "Ambient Light", img: "./img/ambient-light.jpg", time: "1 Dag", price: "Op aanvraag", desc: "Creëer de perfecte sfeer met premium LED verlichting." },
    { title: "Chip Tuning", img: "./img/chip-tuning.jpg", time: "Verschilt per project", price: "Op aanvraag", desc: "Software optimalisatie voor maximale prestaties en efficiëntie." },
    { title: "Starroof", img: "./img/starroof.jpg", time: "4 Dagen", price: "Op aanvraag", desc: "Een sterrenhemel in uw auto." },
    { title: "Rim Painting", img: "./img/rim-painting.jpg", time: "1-2 Dagen", price: "Op aanvraag", desc: "Professionele renovatie en poedercoaten van uw velgen." },
    { title: "Bodykits", img: "./img/bodykits.jpg", time: "Verschilt per project", price: "Op aanvraag", desc: "Transformeer de look van uw auto met sportieve bumpers en spoilers." },
    { title: "Vehicle Wraps", img: "./img/wraps.jpg", time: "3-5 Dagen", price: "Op aanvraag", desc: "Volledige kleurverandering, chrome delete of lakbescherming (PPF)." },
    { title: "Custom Seatbelts", img: "./img/seatbelts.jpg", time: "1 Dag", price: "Op aanvraag", desc: "Gekleurde veiligheidsgordels voor een uniek accent in uw interieur." },
    { title: "Interior Upholstery", img: "./img/interior-upholstery.jpg", time: "1-3 Weken", price: "Op aanvraag", desc: "Herbekleding van sturen en stoelen met premium Nappa of Alcantara." },
    { title: "Brake Calipers", img: "./img/brake-calipers.jpg", time: "1-2 Dagen", price: "Op aanvraag", desc: "Sportieve look met hittebestendige lak en nieuwe logo's." },
    { title: "Steering Wheels", img: "./img/steering-wheels.jpg", time: "Verschilt per project", price: "Op aanvraag", desc: "Custom carbon en LED sturen voor de ultieme rijervaring." },
    { title: "Parts Installation", img: "./img/parts-installation.jpg", time: "Verschilt per project", price: "Op aanvraag", desc: "Professionele montage van uitlaten, spoilers en diffusers." }
];
...
function initServicesSlider() {
    ...
    const infiniteServices = [...servicesData, ...servicesData, ...servicesData, ...servicesData, ...servicesData];
    slider.innerHTML = '';
    ...
    setTimeout(() => {
        ...
        if (nextBtn) {
            nextBtn.onclick = () => {
                ...
                if (slider.scrollLeft >= oneSetWidth * 4 - 50) slider.scrollLeft = oneSetWidth * 2 - 50;
        ...
        if (prevBtn) {
            prevBtn.onclick = () => {
                const w = getCardWidth();
                if (slider.scrollLeft <= oneSetWidth + 50) slider.scrollLeft = oneSetWidth * 3 + 50;
                slider.scrollBy({ left: -w, behavior: 'smooth' });
            };
        }
        slider.addEventListener('scroll', () => {
            const current = slider.scrollLeft;
            if (current < oneSetWidth - 500) slider.scrollLeft = current + oneSetWidth;
            else if (current > oneSetWidth * 4 + 500) slider.scrollLeft = current - oneSetWidth;
        });
...
document.addEventListener('DOMContentLoaded', () => {
    const modalDataDB = {
        'ambient': {
            title: "Ambient Light", price: "€299 - €599", badge: "Instant Booking",
            ...
            features: ["64 Kleuren (RGB)", "App Bediening", "OEM Look & Feel", "Verborgen Installatie", "1 Jaar Garantie"]
        },
        'tuning': {
            title: "Chip Tuning", price: "€599 - €899", badge: "Populair",
            ...
            features: ["Meer PK & Nm", "Zuiniger Brandstofverbruik", "Software op Maat", "Veilig & Betrouwbaar", "Testrit Inbegrepen"]
        },
        'bodykits': {
            title: "Bodykits", price: "Op aanvraag", badge: "Custom",
            ...
            features: ["Agressieve, Sportieve Look", "Premium Merken (Maxton, Brabus, etc.)", "Professionele Montage", "Perfecte Pasvorm"]
        },
        'starroof': {
            title: "Starroof", price: "€1299 - €2499", badge: "Exclusive",
            ...
            features: ["Tot 1000+ Sterren", "Vallende Sterren Effect", "Verschillende Kleuren Instelbaar", "Onzichtbare Installatie in Dakhemel"]
        },
        'rims': {
            title: "Rim Painting", price: "€700", badge: "Instant Booking",
            ...
            features: ["Kras- en Stoeprandschade Herstel", "Professioneel Poedercoaten", "Alle Kleuren en Finishes (Mat/Glans)", "Duurzame en Slijtvaste Afwerking"]
        },
        'wraps': {
            title: "Vehicle Wraps", price: "Op aanvraag", badge: "Styling",
            img: "./img/wraps.jpg",
            ...
            features: ["Volledige Kleurverandering", "Chrome Delete (Zwarte Accenten)", "Lakbescherming (PPF)", "Makkelijk Verwijderbaar", "Premium Folies (3M, Avery)"]
        },
        'seatbelts': {
            title: "Custom Seatbelts", price: "€399 - €699", badge: "Interior",
            ...
            features: ["Keuze uit 50+ Kleuren", "Voldoet aan Originele Veiligheidsnormen", "Snelle Installatie", "Perfect en Uniek Interieurdetail"]
        },
        'upholstery': {
            title: "Interior Upholstery", price: "Op aanvraag", badge: "Luxury",
            ...
            features: ["Premium origineel Alcantara", "Hoogwaardig Nappa Leer", "Custom Stiksels & Patronen", "Volledig Op Maat Gemaakt"]
        },
        'calipers': {
            title: "Brake Calipers", price: "€499 - €899", badge: "Detailing",
            ...
            features: ["Hittebestendige Premium Lak", "Custom Logo's (AMG, M, Porsche, etc.)", "Sportieve Look", "Lange Levensduur & Makkelijk Schoon te maken"]
        },
        'steering': {
            title: "Steering Wheels", price: "Op aanvraag", badge: "Performance",
            ...
            features: ["Echt Carbon Fiber Accenten", "Ingebouwde LED Shift Lights", "Bekleed met Alcantara of Geperforeerd Leer", "Ergonomische en Dikke Grip"]
        },
        'parts': {
            title: "Parts Installation", price: "Op aanvraag", badge: "Workshop",
            ...
            features: ["Montage van Uitlaten & Downpipes", "Luchtinlaatsystemen (Intakes)", "Ophanging & Verlagingsveren", "Professioneel en Veilig Gemonteerd"]
        }
    };
